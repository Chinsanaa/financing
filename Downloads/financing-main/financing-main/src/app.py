"""
Unified Streamlit App: Dashboard + Labeling Interface
Consolidates Flask web app, standalone Streamlit dashboard, and labeling workflows
into a single, cohesive 6-tab analytics and management interface.
"""
...
from translate import enrich_label_row, description_label_english, merchant_label_english
from trends import (
...
def detect_anomalies(df_filtered):
    ...
    one_off = valid[valid['merchant'].isin(merchant_counts[merchant_counts == 1].index)]
    one_off['flag_reason'] = 'One-off merchant'
...
def compute_need_want_split(df_in, budget_config):
    need_cats = [
        c for c, i in budget_config['categories'].items() if i['type'] == 'Need'
    ]
    want_cats = [
        c for c, i in budget_config['categories'].items() if i['type'] == 'Want'
    ]
    need = float(df_in[df_in['category'].isin(need_cats)]['amount'].sum())
    want = float(df_in[df_in['category'].isin(want_cats)]['amount'].sum())
    total = need + want
    return need, want, total


def cumulative_savings_series(df_in, monthly_income):
    monthly = df_in.groupby('month')['amount'].sum().reset_index()
    monthly['spend'] = monthly['amount']
    monthly['savings'] = monthly_income - monthly['spend']
    monthly['month_dt'] = monthly['month'].dt.to_timestamp()
    monthly['cum_savings'] = monthly['savings'].cumsum()
    return monthly


def split_high_value_and_one_off(df_in):
    anomalies = detect_anomalies(df_in)
    if len(anomalies) == 0:
        return pd.DataFrame(), pd.DataFrame()
    high_val = anomalies[anomalies['flag_reason'].str.contains('High value', na=False)]
    one_off = anomalies[anomalies['flag_reason'].str.contains('One-off', na=False)]
    return high_val, one_off
...
if not DATA_PATH.exists():
    st.warning("📁 No classified transactions found. Please run the onboarding first.")
    st.info("""
    **Getting started:**
    1. Prepare your Alipay/WeChat CSV exports
    2. Run `python src/bootstrap.py` in the CLI

    Then come back here to see your dashboard.
    """)
    st.stop()
...
st.markdown(
    ...
    'Personal Finance Categorizer · Logistic Regression + Semantic Embeddings · Graduated Trust'
    '</div>',
    unsafe_allow_html=True,
)